#!/usr/bin/python
#USAGE: python2 run.py
import socket
import sys
import os
import subprocess
# Allow multiprocessors
use_multiprocessing = True
if use_multiprocessing:
    import multiprocessing
    max_cpus = 5 # We might want to not run on the full number of cores, as Rosetta take about 2 Gb of memory per instance
rosetta_scripts_path = os.path.expanduser("/scratch/oscarRB/ProyectoKv7.2/rosetta/software/main/source/bin/rosetta_scripts.static.linuxgccrelease")
nstruct = 50 # Normally 35
max_minimization_iter = 5000 # Normally 5000
abs_score_convergence_thresh = 1 # Normally 1.0
number_backrub_trials = 50000 # Normally 35000
backrub_trajectory_stride = 5000 # Can be whatever you want, if you would like to see results from earlier time points in the backrub trajectory. 7000 is a reasonable number, to give you three checkpouints for a 35000 step run, but you could also set it to 35000 for quickest run time (as the final minimization and packing steps will only need to be run one time).
path_to_script = 'rosetta_scripts/flexddG.wt.xml'
...
#CAMBIAR ESTO
    flex_ddg_args = [
        ...
        '-parser:script_vars soft_pack_sfxn=soft_rep_design.wts ',
        'chainstomove=' + chains_to_move,
        'mutate_resfile_relpath=' + os.path.abspath( os.path.join( input_path, 'C122Y.resfile' ) ),
